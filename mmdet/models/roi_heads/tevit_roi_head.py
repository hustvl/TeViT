            ...
            for i in range(B):
                ...
                for j in range(T):
                    sampling_result.append(self.bbox_sampler[stage].sample(
                        assign_result[j], proposal_list[i * T + j],
                        gt_bboxes[i * T + j]))
    ...
    def simple_test(self,
        ...
        for img_id in range(num_imgs):
            ...
            bbox_pred_per_img = proposal_list[img_id][topk_indices //
                                                      num_classes]
            attn_feats_per_img = bbox_results['attn_feats'][img_id][
        ...
        if self.with_mask:
            ...
            mask_results = self._mask_forward(stage, x, rois, attn_feats)
            mask_results['mask_pred'] = mask_results['mask_pred'].reshape(
                num_imgs, -1, *mask_results['mask_pred'].size()[1:])
            segm_results = []
            ...
            for img_id in range(num_imgs):
                ...
                #     1, num_classes, 1, 1)
                mask_pred_per_img = mask_pred[img_id]
